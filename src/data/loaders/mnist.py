...
Downloads and prepares MNIST dataset using torchvision.
"""
...
class MNISTLoader(DatasetLoader):
    ...
    Downloads MNIST from torchvision and converts to numpy format.
    """
    ...
    def load(
        ...
        **params
        ...
        if self.is_cached(output_dir):
            logger.info(f"MNIST already cached at {output_dir}")
            return self._load_info(output_dir)
        
        logger.info(f"Downloading MNIST to {download_dir}")
        ...
        logger.info(f"MNIST prepared: {info.train_samples} train, {info.test_samples} test")
        return info
