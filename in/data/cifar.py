...
class CustomCIFAR10(data.Dataset):
    ...
    def __getitem__(self, index):
        img = self.data[index]  # likely (3, 32, 32) and float32
...
#         if self.train:
#             self.data = np.load(f"{root}/data.npy")
#             self.labels = np.load(f"{root}/labels.npy")
            
#             if noise_type != 'clean':
#                 # Reshape labels to shape (N, 1) for compatibility
#                 self.labels = self.labels.reshape(-1, 1)
#                 self.noisy_labels, self.actual_noise_rate = noisify(
#                     dataset='cifar10',
#                     train_labels=self.labels,
...
#             self.labels = np.load(f"{root}/test_labels.npy")
