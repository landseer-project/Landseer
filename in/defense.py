...
from data.cifar import CustomCIFAR10
...
def accuracy(logit, target, topk=(1,)):
    ...
    res = []
    for k in topk:
        #Change: Replacing this based on error 1
...
def train(train_loader,epoch, model1, optimizer1, model2, optimizer2):
    ...
    for i, (images, labels, indexes) in enumerate(train_loader):
        ...
        if (i+1) % args.print_freq == 0:
            # Change: Replacing this based on error 3
...
def main():
    ...
    cnn1 = config(input_channel=input_channel, n_outputs=num_classes)
    cnn1.cuda()
    # print(cnn1.parameters)
    optimizer1 = torch.optim.Adam(cnn1.parameters(), lr=learning_rate)
    
    cnn2 = config(input_channel=input_channel, n_outputs=num_classes)
    cnn2.cuda()
